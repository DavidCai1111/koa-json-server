...
router.get('/:key', function*() {
  ...
  let qs = querystring.parse(this.querystring);
  if (config[key] === undefined) {
    yield this.body = {
      errorMessage: `resource: ${key} not exist`
    };
  } else if (qs !== {} || qs !== undefined) {
    let entity;
    if ((entity = tools.findEntityByProps(config[key], qs)) === undefined) {
      yield this.body = {
        errorMessage: `can not find the entity by ${qs}`
      };
    } else {
